...
(define (adiciona-prefixo pre lista-assuntos)
  (if (empty? lista-assuntos)
      '()
      (if (string=? "" (first lista-assuntos))
          (map (lambda (s) (format "[~a]" pre)) lista-assuntos)
          (map (lambda (s) (format "[~a] ~a" pre s)) lista-assuntos))))
...
(define (or-lista lst)
  (fold-right (lambda (a b) (or a b)) #f lst))
...
(define (fold-left f val lst)
  (if (empty? lst)
      val
      (f (fold-left f val (rest lst)) (first lst))))
...
(define (pontuacao-final-fr ini lstded)
  (if (empty? lstded)
      ini
      (fold-right (lambda (a b) (- b a)) ini lstded)))
...
(define (concat-lista lista-str)
  (if (empty? lista-str)
      ""
      (fold-right string-append "" lista-str)))

;;(trace concat-lista)
;; quando fold left é utilizado, a lista é concatenada na ordem reversa
;; ex: "wo3" "shi4" "lao3" "shi1" fica shillao3shi4wo3
...
(define (concat-lista-sep lista-str sep)
  (if (empty? lista-str)
      ""
      (string-append (first lista-str)
                     (fold-left (lambda (a b) (string-append a sep b))
                                ""
                                (reverse(rest lista-str))))))
