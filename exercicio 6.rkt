...
;; Aluna: Karoline Neves Bernando - 11311869 - Ciencia da Computacao
...
(define (filtra-par-lg lst)
  (cond [(empty? lst) '()]
        [(integer? (first lst))
         (if (even? (first lst))
           (cons (first lst) (filtra-par-lg (rest lst))) 
           (filtra-par-lg (rest lst)))]
        [else (filtra-par-lg (rest lst))] ))

;;(trace filtra-par-lg)


 ;; lst pode conter valores de qualquer tipo
...
(define (filtra-comp p lst)
  (cond [(empty? lst) '()]
        [(p (first lst)) (filtra-comp p (rest lst))]
        [else (cons (first lst) (filtra-comp p (rest lst)))]
        ))
...
;;
(define (todos? p lst)
  (foldl (lambda (b1 b2) (and b1 b2)) #t (map p lst)))
...
;;
(define (algum? p lst)
  (foldl (lambda (b1 b2) (or b1 b2)) #f (map p lst)))
...
(define (partition p lst)
  (cond [(empty? lst) '(() ())]
        [(p (first lst))
         (define res (partition p (rest lst)))
         (list
