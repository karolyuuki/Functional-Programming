...
(define (msg-n str n [aux 0])
  (if (= n 0)
      (displayln (~a str aux))
      (begin (displayln (~a str aux ))
       (msg-n str (sub1 n) (add1 aux)))))
...
(define (soma-vetor v)
  (define soma 0)
  (for ([i (in-range (vector-length v))])
    (set! soma (+ soma (vector-ref v i))))      ;; alterar
  soma)
...
(define (prod-vetor vec)
  (for/fold ([prod 1])     
            ([x vec])     
    (* prod x))) 

;; b) o menor valor entre os números de um vetor
(define (min-valor vec)
  (for/fold ([min (vector-ref vec 0)])
            ([x vec])
    (if (< x min)
        (set! min x)
        (set! min min))))
...
(define (maior-str vec)
  (for/fold ([str ""])
            ([x vec])
    (begin (if (> (string-length x) (string-length str))
               (set! str x)
               (set! str str)))))
...
(define-test-suite test-for-fold
  (test-equal? "produto alguns números" (prod-vetor (vector 1 2 3 4)) 24)
  (test-equal? "menor valor" (min-valor (vector 11 2 63 4)) 2)
  (test-equal? "algumas strings"
               (concatena-str (list "Tokyo" "Osaka" "Nara"))
               "TokyoOsakaNara")
  (test-equal? "Algumas strings"
               (maior-str (list "Lost" "Fringe" "The Sopranos"))
               "The Sopranos"))
